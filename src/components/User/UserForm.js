...
const UserForm = () => {
  ...
  const passwordErrorMessage='the password field should not be empty and should have at least one number, one special char, one lower case and uppercase char  and the number of chars should be between 6 and 16';
  return (
    <Card>
      
       <form onSubmit={submitHandler} className={classes["form-wrapper"]}>
      {error && <p className={classes.error}>{error}</p>}
      <Input label="Password" className={classes.control} isValid={passwordHasError} input={{
      ...
      {!isLoading && <button disabled={!formIsValid}>Change password</button>}
